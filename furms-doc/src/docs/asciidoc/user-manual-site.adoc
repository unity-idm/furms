...
For all the presented messages administrator can check the contents of the request's body. It is also possible to re-send any message, and to cancel (drop) it.

Cancellation can be used to unblock some parts of UI, which are blocked waiting for an operation to be finished, when it is certain that the requested operation will not be served on site agent (e.g. due to its crash or other bug). 

WARNING: Using cancel and resend functions should be used as the last resort. FURMS handles such interruptions of the regular message flow on the best effort basis. Therefore canceling and resend of requests should be performed only when it is certain that site agent is not going to properly handle the original request.     
