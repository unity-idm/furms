...
* `resourceUsageAlarm` - utilized to send information to project administrators, that particular resource usage alarm reached configured threshold. 
* `resourceUsageAlarmWithoutUrl` - utilized to send information that particular resource usage alarm reached configured threshold. This template is used to notify individually picked users configured on resource Alarm level. 
...
| Project allocation name for which alarm has been configured

| `${custom.alarmName}`
| Name of the alarm that reached configured threshold

| `${custom.furmsUrl}`
| URL to FURMS that points to project, which reached alarm threshold
...
The following sections provides detailed information on steps that needs to be taken in order to backup the system, and the steps to restore it from scratch basing on the backuped data.

NOTE: FURMS devops tooling comes with handy Ansible Playbooks to perform backup and restore operations, therefore all the operations are performed on Ansible management hosts. The backup files are stored under the directory pointed out by `backupDir` parameter in the Ansible group vars configuration file. It is up system administrator to store backups in a secure place.
...
Depending upon whether you are restoring the system from scratch or restoring the system state from a backup, the steps taken by administrator differs.
...
* Validate whether Ansible group vars file needs reconfiguration e.g. database host and credentials for Unity-IdM and FURMS service are correct, or Broker configuration is valid taking into account current environment setup.
