...
FURMS utilize Unity-IdM notification subsystem, therefore in order to setup email templates used by FURMS, administrator has to create the Unity-IdM configuration files, descried in the next subsections, and put a reference to those in the Ansible group variable file.

==== Message templates configuration
This particular configuration file is parsable by Unity-IdM and defines all email template definitions, here is the default content:
...
The path to this file shall be provided in the  `unity.msgTemplates.configuration` property value in the Ansible groups variable file. Note that there are couple of predefined message template definitions that must be preserved. The following template definition are used by FURMS:

* `policyAcceptanceNew` - utilized to notify user about new policy document to accept,
* `policyAcceptanceRevision` - utilized to notify user that existing policy has been updated and waits approval,
* `invitationNew` - utilized to notify user about invitation to accept new role in FURMS,
* `registrationInvitation` - utilized to notify user about invitation to become user in FURMS with particular role.

Most of the configuration in the default message template file is fixed. Only the template subject is allowed to be changed by the FURMS system administrator.
...
Email templates use custom variables that defines dynamic data set by FURMS when sending notifications. The following provides information on custom variables, its description and where are used:

* `${custom.name}` - can be used in `policyAcceptanceNew` or `policyAcceptanceRevision` HTML template files, and is resolved to the policy document name,
* `${custom.role}` - can be used in `invitationNew` or `registrationInvitation` HTML template files, and is resolved to the FURMS role that invited user is about to accept,
* `${custom.furmsUrl}` - can be used by all of the HTML templates except `registrationInvitation`, and points to the FURMS application,
* `${url}` - can be used in `registrationInvitation` HTML template file and points to the Unity-IdM registration form.
