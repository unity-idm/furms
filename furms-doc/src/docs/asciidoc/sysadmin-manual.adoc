...
The communication between FURMS and particular site is done through two queues:

* dedicated one used for particular site, created by FURMS during site's creation,
* `reply-queue` which is fixed, created out of the box by RabbitMQ, and used by all sites to publish messages to FURMS.

The dedicated queue name is visible on Site administrator's Settings view as 'Site Id'.

From Site's point of view, broker operation must define the client user name and password with a connection rights to FURMS virtual host, and privileges to:

* read/write messages from/to particular sites's queue,
* read/write messages from/to `reply-queue` queue,
