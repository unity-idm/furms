...
package io.imunity.furms.integration.tests.security.user_operation;
...
class UserOperationServiceSecurityTest extends SecurityTestsBase {
	...
	void shouldPassForSecurityRulesForMethodsInResourceUsageCSVExporter() {
		...
		forMethods(() -> service.findCurrentUserSitesInstallations())
			.accessFor(
				basicUser(),
				fenixAdmin(),
				siteAdmin(site),
			...
			.andForMethods(() -> service.findUserAdditionsByProjectId(project))
			.accessFor(
				communityAdmin(community),
				projectAdmin(community, project),
				projectUser(community, project)
