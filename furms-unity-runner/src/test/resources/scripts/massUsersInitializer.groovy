...
@Field final int ADMIN_IN_GROUPS = 0;
@Field final int USER_IN_GROUPS = 30;
@Field final int ENTITIES = 30;
@Field final int GROUPS = 10;
...
void addUsersToGroups()
{
	...
	for (int j=0; j<1; j++)
	{
		...
		for (int i=0; i<50; i++)
		{
			Group grp = new Group("/root/grpx" + i);
			list.add(grp);
		}
	...
	for (int j=0; j<50; j++)
	{
		...
		{
			int entityNo = j*10 + i;
			memberships.add(new GroupMembership("/root/grpx" + j, entityNo+2, date));
		}
		tx.runInTransaction(new TxRunnable()
		{
			public void run() {
