...
@Field final int ENTITIES = 300;
@Field final int IN_BATCH = 100;
...
void createExampleUsers()
{
	...
	for (int j=0; j<ENTITIES; j++)
	{
		...
		for (int i=0; i<IN_BATCH; i++)
		{
			...
			def persistent = UUID.randomUUID().toString();
			identities1.add(new StoredIdentity(new Identity(UsernameIdentity.ID, value,
					ids.get(i), value)));
			identities1.add(new StoredIdentity(new Identity("persistent", persistent,
					ids.get(i), persistent)));
			identities1.add(new StoredIdentity(new Identity("identifier", persistent + "@fenixId@acc.fenix.eduteams.org",
					ids.get(i), persistent + "@fenixId@acc.fenix.eduteams.org")))
		}
		...
		for (int i=0; i<IN_BATCH; i++)
		{
			...
			Attribute createdInScript = StringAttribute.of(NAME_ATTR, "/", "createdInScript");
			attributes.add(new StoredAttribute(new AttributeExt(createdInScript, true), ids.get(i)))
		}
...
void addUsersToGroups()
{
	...
	for (int j=0; j<ENTITIES; j++)
	{
		...
		for (int i=0; i<IN_BATCH; i++)
		{
			...
			memberships.add(new GroupMembership("/fenix/users", entityNo+2, date));
		}
		tx.runInTransaction(new TxRunnable()
				{
					public void run() {
