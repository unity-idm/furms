...
public interface UserGrantEntityRepository extends CrudRepository<UserGrantEntity, UUID> {
	...
	@Query(
		"select ua.*, s.external_id as site_external_id " +
			...
			"where ua.site_id = :site_id and ua.project_allocation_id = :project_allocation_id"
	)
	Set<UserGrantReadEntity> findBySiteIdAndProjectAllocationId(@Param("site_id") UUID siteId, @Param("project_allocation_id") UUID projectAllocationId);
