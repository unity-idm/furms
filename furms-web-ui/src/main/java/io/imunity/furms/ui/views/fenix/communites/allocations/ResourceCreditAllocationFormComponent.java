...
public class ResourceCreditAllocationFormComponent extends Composite<Div> {
	private static final int MAX_NAME_LENGTH = 20;

	private final Binder<ResourceCreditAllocationViewModel> binder;
	...
	ResourceCreditAllocationFormComponent(Binder<ResourceCreditAllocationViewModel> binder, ResourceCreditAllocationComboBoxesModelsResolver resolver) {
		this.binder = binder;
		...
		resourceTypeComboBox.addValueChangeListener(event -> {
			resourceCreditComboBox.setItems(resolver.getResourceCredits(event.getValue().id));
		});
		...
		Label availableAmountLabel = new Label();
		resourceCreditComboBox.addValueChangeListener(event ->
			availableAmountLabel.setText(getTranslation("view.fenix-admin.resource-credits-allocation.form.label.available") + event.getValue().amount)
		);
	...
	private void prepareValidator(TextField nameField, ComboBox<SiteComboBoxModel> siteComboBox,
		...
		binder.forField(nameField)
			...
			.bind(ResourceCreditAllocationViewModel::getName, ResourceCreditAllocationViewModel::setName);
		binder.forField(siteComboBox)
			...
			.bind(
				ResourceCreditAllocationViewModel::getSite,
				ResourceCreditAllocationViewModel::setSite
			);
		binder.forField(resourceTypeComboBox)
			...
			.bind(
				ResourceCreditAllocationViewModel::getResourceType,
				ResourceCreditAllocationViewModel::setResourceType
			);
		binder.forField(resourceCreditComboBox)
			...
			.bind(
				ResourceCreditAllocationViewModel::getResourceCredit,
				ResourceCreditAllocationViewModel::setResourceCredit
			);
		binder.forField(amountField)
			...
			.bind(
				ResourceCreditAllocationViewModel::getAmount,
				ResourceCreditAllocationViewModel::setAmount
			);
	}
	...
	public void setFormPools(ResourceCreditAllocationViewModel model) {
		if(model.site != null) {
